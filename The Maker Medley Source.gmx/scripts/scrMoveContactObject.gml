...
var t_xamt, t_yamt, t_limit, t_target;
...
for (var i = 0; i <= ceil(t_limit); i += 1) {
    if (place_meeting(x+t_xamt*i, y+t_yamt*i, t_target)) {
        ...
        if (i > 1){ //if i is 0, the instance running the code was already in the object; if 1, the instance was already aligned with the object
            x += t_xamt*(i-1);
            y += t_yamt*(i-1);
        }
        return t_col;
