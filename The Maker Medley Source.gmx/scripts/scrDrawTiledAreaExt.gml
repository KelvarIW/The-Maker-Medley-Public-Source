/// scrDrawTiledAreaExt(background,xOrigin,yOrigin,x1,y1,x2,y2, xscale,yscale,color,alpha)
//
...
var bg,xx,yy,x1,y1,x2,y2,_xscale,_yscale,_col,_alpha;
bg = argument[0];
xx = argument[1];
yy = argument[2];
x1 = argument[3];
...
for(i=i; i<=x2; i+=bw) {
    for(j=j; j<=y2; j+=bh) {
        ...
        draw_background_part_ext(bg,left,top,width,height,X,Y,_xscale,_yscale,_col,_alpha);
    }
    j = jj;
}
